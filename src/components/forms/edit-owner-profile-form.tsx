...
import type { OwnerProfile, User } from "@/types";
...
export function EditOwnerProfileForm({ ownerProfile, user }: EditOwnerProfileFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      companyNif: ownerProfile.companyNif || "",
      companyAddress: ownerProfile.companyAddress || "",
      description: ownerProfile.description || "",
      latitude: ownerProfile.latitude || 0,
      longitude: ownerProfile.longitude || 0,
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
        const profileRef = doc(db, "ownerProfiles", ownerProfile.id);
        await updateDoc(profileRef, values);

        toast({
            title: "Profile Updated!",
            description: "Your owner profile details have been successfully saved.",
        });
  ...
  return (
    <Card>
        <CardHeader>
            <CardTitle>Edit Your Owner Profile</CardTitle>
