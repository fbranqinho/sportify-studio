...
import { Bell, LogOut, Settings, Gem } from "lucide-react";
...
function NotificationBell({ onListenerInit }: { onListenerInit?: (unsubscribe: () => void) => void }) {
    const { user } = useUser();
    ...
    React.useEffect(() => {
        ...
        if (onListenerInit) onListenerInit(unsubscribe);

        return () => unsubscribe();
    }, [user?.id, onListenerInit]);
                            ...
                            <div className="space-y-4">
                                {notifications.slice(0, 10).map(n => (
                                    <Link
                                        href={n.link}
                                        key={n.id}
                                        className="block hover:bg-muted -mx-2 px-2 py-2 rounded-md"
                ...
                <Sidebar variant="sidebar" collapsible="icon">
                    <SidebarHeader className="items-center justify-center gap-2 data-[collapsible=icon]:-ml-1 data-[collapsible=icon]:justify-center">
                        <Link href="/" className="flex items-center gap-2 font-headline text-2xl font-bold text-primary">
                            ...
                            <span className="duration-300 group-data-[collapsible=icon]:hidden">Sportify</span>
                        </Link>
                    </SidebarHeader>
                    <SidebarContent>
                ...
                <div className="flex flex-1 flex-col">
                    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background/80 px-4 backdrop-blur-sm sm:px-6">
                        ...
                        <div className="flex items-center gap-2">
                            <NotificationBell
                                onListenerInit={(unsubscribe) => listenersRef.current.push(unsubscribe)}
                            />
                        </div>
                    </header>
